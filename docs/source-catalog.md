...
| MPContribs | Materials contributed data | platform / API | P0 | Implemented (page-level) | contributed experiments and project data | `StructuredRecord` |
...
| AFLOW | Structured materials DB | REST / AFLUX | P1 | Implemented (page-level) | large-scale materials properties | `StructuredRecord` |
...
| COD | Crystal structure database | search / dumps | P1 | Implemented (page-level) | crystal structure coverage | `StructuredRecord` |
...
| MDF | Materials repository | discover / publish | P1 | Implemented (page-level) | curated materials datasets | `StructuredRecord` |
| NIST Materials Data Repository | Materials repository | repository portal | P1 | Implemented (page-level) | open materials datasets | `StructuredRecord` |
| NIST Chemistry WebBook | Chemistry database | web resource | P0 | Implemented (page-level) | physical and chemical properties | `StructuredRecord`, `KnowledgeRecord` |
| PubChem | Chemistry database | PUG REST | P0 | Implemented | compound properties and identifiers | `StructuredRecord`, `KnowledgeRecord` |
| Open Reaction Database | Reaction data | docs / repository | P1 | Implemented (page-level) | machine-readable reaction data | `StructuredRecord`, `InstructionTrace` |
| Open Catalyst Project | Catalysis data | dataset downloads + code | P1 | Implemented (page-level) | catalyst structure / relaxation data | `StructuredRecord` |
| Catalysis-Hub | Catalysis data | web platform | P1 | Implemented (page-level) | surface reaction data | `StructuredRecord`, `KnowledgeRecord` |
| Battery Archive | Battery data | website / csv access | P1 | Implemented (page-level) | cycling and safety data | `StructuredRecord` |
| Wikidata | Open knowledge graph | MediaWiki API | P0 | Implemented | entity descriptions and linked knowledge | `KnowledgeRecord` |
| Wikipedia dumps | Open knowledge text | official dumps | P1 | Implemented (page-level) | encyclopedia-style explanatory text | `Document` |
| ChemLibreTexts | Open education | web pages / PDFs | P1 | Implemented (page-level) | chemistry learning content | `Document` |
| MIT OCW | Open education | course site | P1 | Implemented (page-level) | course notes and assignments | `Document` |
| NPTEL | Open education | course site | P1 | Implemented (page-level) | engineering course content | `Document` |
| CORE | OA aggregator | API | P1 | Implemented (page-level) | aggregated open-access papers | `Document` |
...
| PATENTSCOPE | Patents | search / api-related access | P1 | Implemented (page-level) | global patent documents | `Document` |
| EPO OPS | Patents | OPS automation path | P1 | Implemented (page-level) | patent search and metadata | `Document` |
